...
#include "../include/yolo_tracker.hpp"//注意双引号
#include <ros/time.h>
...
public:
     ...
     {
        //创建订阅者
        start_tracking_sub = nh.subscribe("/ship",1,&Yolo_sub_pub::start_tracking_callback,this);
        yolo_sub = nh.subscribe("/yolov8/BoundingBoxes",1,&Yolo_sub_pub::yolo_callback,this);
        local_position_sub = nh.subscribe<geometry_msgs::Pose>("/gazebo/zhihang/standard_vtol/local_position",1,&Yolo_sub_pub::local_position_callback,this);
...
//     Kp_z = 1
    ...
    {
        start_tracking=msg;//原来bool值是这样传输的
    } 
     ...
     void yolo_callback(yolov8_ros_msgs::BoundingBoxes data)
    {
        ...
        if( Num_boxes_now == 1)//如果vector的数量没有发生变化，则保持悬停
        {
            target = data.bounding_boxes.back();
            int Num_boxes_now=data.bounding_boxes.size();//识别到才有ector中的成员
            //gazebo/performance_metrics
            if(target.probability >= 0.80)
            {   //回应
                yolo_find_ship = true;
                float delta_x = ((target.xmax+target.xmin)/2-image_center_x); //delta_x控制左右 正左
                float delta_y = ((target.ymax+target.ymin)/2-image_center_y); //delta_y控制前后 正后
                ...
                else
                {
                    ...
                    twist.linear.y = 0;//-y_velocity;
                    twist.linear.z = 0;

                }
                std::cout << delta_x*delta_x << " " <<delta_y*delta_y << std::endl;
                if(delta_x*delta_x + delta_y*delta_y <=10000)
                {
                    twist.linear.x = 0;
                    twist.linear.y = 0;
                    twist.linear.z = -2;
        ...
        if(local_station.position.z >=16)
        {
            cmd_vel_pub.publish(twist);
            ROS_INFO("Yolo_Is_Traccking"); //一定要用上双引号
